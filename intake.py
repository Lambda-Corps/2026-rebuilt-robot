...
class Intake(Subsystem):
    def __init__(self):
        ...
        self.intake_duty_cycle_out = controls.DutyCycleOut(
            0.0
        )  # Instantiate speed control object
    ...
    def intake_speed_global_control(self) -> None:
        if self.intake_enabled:  # Global control of intake speed
            self.intake_duty_cycle_out.output = (
        ...
        log_smartdashboard_number(
            "Intake Velocity: ", velocity_value, min_verbosity=10, update_frequency=10
        )
        log_smartdashboard_boolean(
            "Intake Enable: ",
            self.intake_enabled,
    ...
    def enable_intake(self, enable):
        ...
        log_debug(f">>>> self.intake_enabled {self.intake_enabled}", min_verbosity=1)
