...
class Shooter(Subsystem):
    ...
    def flywheel_spin(self, flywheel_spinspeed: float) -> None:
        ...
        log_debug(f"velocity_value: {velocity_value:6.2f}", min_verbosity=10)

    def flywheel_spin_global_control(
        self,
    ) -> None:  # Speed not being controlled by parameter
        if self.flywheel_enabled:  # Global control of Flywheel
            self.counter = self.counter + 1

            self.flywheel_duty_cycle_out.output = (
                self.motor_speed_global
            )  # Speed set by global variable
            self._shooter_flywheel.set_control(self.flywheel_duty_cycle_out)
    ...
    def change_speed_variable_function(self, speed_update: float) -> None:
        if (self.motor_speed_global > -1) and (self.motor_speed_global < 1):
            ...
            log_debug(
                f">>>>> self.motor_speed_global {self.motor_speed_global}   Subsystem",
                min_verbosity=5,
            )
    ...
    def enable_flywheel(self, enable):
        ...
        log_debug(
            f">>>> self.flywheel_enabled {self.flywheel_enabled}", min_verbosity=1
        )
